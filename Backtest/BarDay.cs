    ...
    public class BarDay : IBarDay
    {
        ...
        IBar IBarDay.GetMinuteBar(int indi, int minutes, int p)
        {
            ...
            if (minutes == 5)
            {
                ...
                ind *= 8;
                ind += (390 * 8);
                arrOff = this.offset + ind;
            }

            Bar rv = new Bar(null, 0,0);
            return rv;
        }
        ...
        public short[] arr;
        int underflow = 0;
        BarDay(int day, int offset, string sym, short[] arr)
        {
            this.arr = arr;
            this.offset = offset;
            this.symbol = sym;
            this.date = epoch.AddDays(day);
        }
        public static void Load(string fn)
        {
            ...
            short[] symx = new short[Buffer.ByteLength(indata) / 2];
        ...
        public static int GetSlot(string s, string[] symbols, DateTime[] dates, out int minSlot)
        {
            ...
            return (dateIndex * 390 * 8 * symbols.Count()) + (symbolIndex * 8) + (totalMinutes * 8 * symbols.Count());
        }
        public static short[] BuildYearBin(string dir)
        {
            ...
            if (Int32.TryParse(dir.Split('\\').Last(), out year))
            {
                ...
                arr = new short[symbolCount * 8 * 390 * dates.Count()];
                foreach (string s in strings)
                {
                    ...
                    {
                        int prevMinuteSlot = slot - (symbolCount * 8);
                        double oldDayHigh = BarDay.itod(arr[prevMinuteSlot + 4]);
                        double oldDayLow = BarDay.itod(arr[prevMinuteSlot + 5]);
                    ...
                    //int slotno = (i * 8 * (390 + 78)) + (dayslot * 8);
                    //dayslot++;
                    arr[slot + 0] = dtoi(open);
                    arr[slot + 1] = dtoi(high);
                    arr[slot + 2] = dtoi(low);
                    arr[slot + 3] = dtoi(close);
                    arr[slot + 4] = dtoi(dayHigh);
                    arr[slot + 5] = dtoi(dayLow);
                    ...
                    //}
                    Buffer.BlockCopy(new int[] { volume }, 0, arr, slot * 2 + 12, 4);

                }
            }
            if (arr != null)
            {
                FileStream fs0 = File.OpenWrite(@"c:\src\data\1998.bin");
                byte[] fsdata = new byte[arr.Length * 2];
                Buffer.BlockCopy(arr, 0, fsdata, 0, fsdata.Length);
                fs0.Write(fsdata, 0, fsdata.Length);
                fs0.Close();
                File.WriteAllLines(@"c:\src\data\1998.sym",symbols);
                string[] dateStrings = (from d in dates select d.ToString("MM/dd/yyyy")).ToArray();
                File.WriteAllLines(@"c:\src\data\1998.dates", dateStrings);
            }
            return arr;
        }
        ...
        static Bar[] getNBars(short[] arr, int day, int symbolNo, int period, int symbolCount, int oneMinCandlePerConsolidatedCandle)
        {
            ...
            for (int i = 0; i < oneMinCandlePerConsolidatedCandle; i++)
            {
                ...
                bars[i] = new Bar(arr, off, symbolCount);
            }
            return bars;
        }
        public static short[] BuildConsolidatedBars(string binName, string symbolName, string dateName, int oneMinCandlePerConsolidatedCandle)
        {
            ...
            short[] arr = new short[b.Length / 2];
            ...
            short[] arrC = new short[arr.Length / oneMinCandlePerConsolidatedCandle];
            for (int i = 0; i < consolidatedCandleCount; i++)
            {
                int period = (i / symbols.Count()) % consolidatedCandlesPerDayPerSymbol;
        ...
        public static void validateBinFile(string binName, string symName,string dateName)
        {
            ...
            {
                string symbol = symbols[(i / 8) % symbols.Length]; 
                Bar b = new Bar(sa, i, symbols.Length);
        ...
        public static void validateBinConsolidatedFile(string binName, string symName, string dateName, int oneMinCandlePerConsolidatedCandle)
        {
            ...
            foreach (string sym in symbols)
            {
                File.WriteAllLines($@"c:\src\data\val\{sym}{oneMinCandlePerConsolidatedCandle}.txt", valData[sym].ToArray());
            }
        }
        ...
        public static void setInt(int val, short[] arr, int offset)
        {
            int[] ia = { val };
            Buffer.BlockCopy(ia, 0, arr, offset * 2, 4);
        }
        public static int getInt(short[] arr, int offset)
        {
            int[] dst = new int[1];
        ...
        public static void BuildSymbolBin(string sym)
        {
            ...
            short[] dayMap = new short[numDays * 3];
            for (int i = 0; i < numDays; i++)
            {
                IGrouping<string, string> day = daylist[i];
                string key = day.Key;
